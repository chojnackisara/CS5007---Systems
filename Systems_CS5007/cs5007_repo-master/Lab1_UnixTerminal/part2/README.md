...
The numbers in the code, presumably to allocate memory, were all doubled.
...
It looks like movq, which uses 64 bits, is used more often in swap.s. swap int predominantly just uses movl, which uses 32 bits.
...
It seems to be offset by -1600, and each place in the array after that is +4 from that.
...
The dynamic array calls malloc and uses the call "addq".
...
There are two unconditional jump statements, one jump not equal statement, and one jump less/equal statement.

#e Compiler generated assembly 6 - For-loops
...
The code for the condition statement goes at the end.
...
i- See what code a switch statement generates. Is it optimal?

### Response/Observations

It is not optimal because it creates multiple calls to different groups of instructions.
...
Popq is called at the end of the add function as well as at the end of the main function.
