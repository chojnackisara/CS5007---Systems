...
1. The bug was a segmentation fault. 
2. The bug occurred on line 15.
3. The "initialize front" method accesses memory that it is not supposed to, so this needs to be altered.
...
Pro:
1. One pro of this is that it saves time; you don't have to wait for the program to finish running before doing the analysis.


Con:
1. One con of this is that certain specific information might be missed that you cannot predict. Or you might not be able to predict how long the program will run for. It would also be time consuming if conducted manually.
...
Pro:
1. One pro of this is that you have all the information you need since the program has already run. You can see if there were issues while running that might not have been present or evident before. 

Con:
1. One con of this is that it could be more difficult to trace the vulnerability back to the exact location in the code, which would take longer to fix the problem.
