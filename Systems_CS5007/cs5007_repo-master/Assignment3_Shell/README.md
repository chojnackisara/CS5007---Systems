...
The calculator takes two numbers and an operator, and calculates the outcome of the equation.

DESCRIPTION:

The calculator first asks for the operator, which can be chosen from addition, subtraction, 
multipication, and division. Then the user has to specify two numbers separated with a space.
The calculator will output the whole equation and the solution on the screen.

NOTES:

I thought of something that might be useful for someone using a computer. Immediately, a 
calculator came to mind. So I decided to make a calculator that presents output nicely
and quickly as a useful tool for anyone wanting to do quick math.
