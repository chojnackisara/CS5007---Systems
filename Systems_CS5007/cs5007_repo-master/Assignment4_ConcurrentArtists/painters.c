...
void* paint(void* args){
	...
	for(int i =0; i < 5000; ++i){
        	...
        	if (pthread_mutex_trylock(&canvas[painter->x][painter->y].lock)) { 
...
int main(){
	...
	int m;
	for (m = 0; m < rookieArtists; m++) {
		artist_t* current = malloc(sizeof(artist_t));
                current->x = rand_r(&seed)%CANVAS_WIDTH-1;
                current->y = rand_r(&seed)%CANVAS_HEIGHT-1;
                current->r = rand_r(&seed)%256;
                current->g = rand_r(&seed)%256;
                current->b = rand_r(&seed)%256;
		moreArtists[m] = current;
		pthread_create(&moreArtists_tid[m], NULL, (void*)paint, current);
		seed += 1;
	}
