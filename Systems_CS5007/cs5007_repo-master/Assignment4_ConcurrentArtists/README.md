...
In this program, starvation could occur if painters keep trying to pain on places that are already painted, because one process has used all the resources. So this may lead to some processes never getting serviced. The blocked threads are wating to get the resource but they never do, therefore they starve to death. We can fix this in the code by using the wait() method which would release the lock but enter a waiting state. This way the other threads are given an equal chance to run. 
...
I caught up on memory allocation. I was confused about when it's appropriate to use dynamic vs. static memory allocation so I read some articles and watched some videos about that. I also learned about "calloc" and "realloc" which I didn't really know anything about before.
